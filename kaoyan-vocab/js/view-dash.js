...
function renderDash(){
  ...
  for(let i=1;i<=PLAN_DAYS;i++){
    ...
    const rev=[i-1,i-3,i-7,i-14].filter(L=>L>=1&&L<=PLAN_DAYS).length*q;
  ...
  for(let i=13;i>=0;i--){ const ts=Date.now()-i*DAY_MS; const k=todayStr(ts); const hh=S.history[k]||{new:0,rev:0}; days.push({k,hh}); maxv=Math.max(maxv,hh.new+hh.rev); }
  ...
  <div class="card hero" style="margin-bottom:16px">
    ...
    <div style="flex:1;min-width:240px">
      <h2 style="margin:0 0 4px">${day<=PLAN_DAYS?`🔥 冲刺 Day ${day} / ${PLAN_DAYS}`:'🎓 巩固期（14天计划已完成）'}</h2>
      <p class="muted" style="margin:0 0 14px">${day<=PLAN_DAYS?`剩余 ${remain} 天 · 每日目标 ${q} 个新词 + 到期复习`:'继续复习巩固，防止遗忘'}</p>
      <div class="grid3">
        <div class="statcard good"><b>${mastered}</b><span>已掌握(≥4次记对)</span></div>
  ...
  <div class="grid2" style="margin-bottom:16px">
    ...
    <div class="card">
      <h3 style="margin:2px 0 10px;font-size:15.5px">📈 近14天学习量</h3>
  ...
  <div class="card">
    ...
    <p class="muted" style="font-size:12.5px;margin:10px 0 0">* 复习量按艾宾浩斯间隔(1/2/4/7/15天)估算，含答错重记的词会更多。进度可在「设置」调整每日词量。</p>
  </div>`;
}
